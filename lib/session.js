...
export function readCookie(header, name) {
  for (const part of String(header || '').split(';')) {
    ...
    if (k !== name) continue;
    const raw = rest.join('=');
    // A corrupted cookie must never throw: decodeURIComponent rejects a stray
    // `%`, and an uncaught URIError in the middleware is a 500 on every gated
    // path — with an HttpOnly cookie the visitor can't clear it themselves.
    // Tokens are base64url, so an undecodable value simply fails to verify.
...
export async function isAuthed(request, env) {
  ...
  // Fail closed rather than throw: a malformed request should land on /login,
  // not on a 500 the visitor has no way out of.
  try {
    return await verifyToken(secretFor(env), readCookie(request.headers.get('cookie'), COOKIE));
  } catch {
    return false;
  }
}
